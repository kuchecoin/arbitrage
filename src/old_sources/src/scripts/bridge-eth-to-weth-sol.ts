...
export async function bridgeEthToWethSol(ethToTransfer: string, wh: Wormhole<"Mainnet">) {
	...
	const source = await getSigner<"Mainnet", "Ethereum">(origChain);
...
async function tokenTransfer(
	wh: Wormhole<"Mainnet">,
	route: {
		token: TokenId<'Ethereum'>;
		amount: bigint;
		source: SignerStuff<"Mainnet", "Ethereum">;
	...
	let xfer: TokenTransfer<"Mainnet">;
	...
	if (xfer.transfer.protocol === 'ExecutorTokenBridge') {
		...
		const dstToken = await TokenTransfer.lookupDestinationToken<"Mainnet", "Ethereum", "Solana">(
			route.source.chain,
