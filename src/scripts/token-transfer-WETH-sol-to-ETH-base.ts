...
(async function () {
	...
	const destination = await getSigner<"Mainnet", "Base">(destChain);
...
async function tokenTransfer(
	...
	route: {
		...
		destination: SignerStuff<"Mainnet", "Base">;
		delivery?: {
	...
	if (xfer.transfer.protocol === 'ExecutorTokenBridge') {
		...
		const dstToken = await TokenTransfer.lookupDestinationToken<"Mainnet", "Solana", "Base">(
			route.source.chain,
